...
헤드리스 로봇에서는 숫자만 봐서는 원인을 못 가른다. 로봇에서 스트림을 띄우고
노트북 브라우저로 본다:
...
잡힌 마커는 **전부** 그린다 — 대상 ID 는 초록, 나머지는 주황. 로그 한 줄이 원인을 가른다:

| 화면/로그 | 원인 | 대응 |
|---|---|---|
| `z=... ex=... yaw=...` | 정상 | 그대로 진행 |
| `id [3] 는 보이는데 1 은 아니다` | ID 불일치 | `--marker-id 3` 또는 마커 교체 |
| `검출 0 \| 사전훑기: DICT_6X6_250[5]` | **사전 틀림** | `--dict DICT_6X6_250` |
| `검출 0 \| 사전훑기: 어느 사전에도 없음` + 밝기 낮음 | 어둡다/초점 | 조명, 렌즈 확인 |
| `검출 0` + 화면에 마커가 아예 안 보임 | 화면 밖·각도 | 마커 위치·카메라 방향 |
| 프레임 자체가 안 옴 | 장치 경합 | 다른 스택(영상송출) 끄기 |

7cm 마커는 화면 폭의 5% 아래로 작아지면(약 1.5m) 코너를 못 잡는다. `size=` 값을 봐라.
